...
/**
 * A selection is an array of elements pulled from the current document. D3 uses CSS3 to select elements. See
 * {@link D3#select(String)} and {@link D3#select(com.google.gwt.dom.client.Node)} methods for creating
 * {@link Selection}.
 * <p>
...
public class Selection extends EnteringSelection {
     ...
     * classes.
     * <p>
     * If add is true, then all elements are assigned the specified class, if not already assigned; if false, then the
     * class is removed from all selected elements, if assigned.
     * 
     * @param className the className to add or remove
     * @param add true to add false to remove the class from all the elements of the selection
     * @return the selection
     */
    public native final Selection classed(String className, boolean add)/*-{
		return this.classed(className, add);
    }-*/;

    /**
     * Sets whether or not the class returned by the given function
     * is associated with the selected element.
     * <p>
     ...
     * class is removed from all selected elements, if assigned.
     * 
     * @param classNameFunction the function evaluated for each element and returning a className for each of them
     * @param add true to add false to remove the class from all the elements of the selection
     * @return the selection
     */
    public native final Selection classed(DatumFunction<String> classNameFunction, boolean add)/*-{
		return this.classed(function(d, i) {
			return classNameFunction.@org.gwtd3.api.functions.DatumFunction::apply(Lcom/google/gwt/dom/client/Element;Lorg/gwtd3/api/core/Datum;I)(this,{datum:d},i);
		}, add);
    }-*/;
     ...
     * currently a way to add them back to the document. (See the {@link #append(String)} and
     ...
     * 
     * 
     * @param data the data array to map to the selection
     * @return
     */
    ...
    /**
     * Joins the specified array of data with the current selection by controlling how the data is mapped to the
     ...
     * <p>
     * 
     * @param array the data array to map to the selection
     * @param keyFunction the function to control how data is mapped to the selection elements
     ...
     * 
     * @param eventType
     *            the type of the event to listen to; prefix the type with a dot to remove all listeners (specifying
     ...
     * @param useCapture
     *            a capture flag, which corresponds to the W3C useCapture flag: "After initiating capture, all events of
     *            the specified type will be dispatched to
     *            the registered EventListener before being dispatched to any EventTargets beneath them in the tree.
