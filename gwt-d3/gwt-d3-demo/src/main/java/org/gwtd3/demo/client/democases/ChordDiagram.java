...
import org.gwtd3.api.arrays.MapNumbersCallback;
import org.gwtd3.api.core.Datum;
import org.gwtd3.api.core.Selection;
import org.gwtd3.api.functions.DatumFunction;
import org.gwtd3.api.layout.ChordLayout;
import org.gwtd3.api.layout.ChordLayout.Chord;
import org.gwtd3.api.layout.ChordLayout.Group;
import org.gwtd3.api.scales.OrdinalScale;
...
public class ChordDiagram extends FlowPanel implements DemoCase {
    ...
    public void start() {
        ...
        DatumFunction<Array<GroupTick>> groupTicks = new DatumFunction<Array<GroupTick>>() {
            ...
            public Array<GroupTick> apply(final Element context, final Datum d, final int index) {
                ...
                return D3.range(0, g.value(), 1000).<Array<Double>> cast().map(new MapNumbersCallback<GroupTick>() {
                    @Override
                    public GroupTick map(final double v, final int i, final Array<Double> array) {
                        double angle = (v * k) + g.startAngle();
                        String label = (i % 5) != 0 ? null : (v / 1000) + "k";
    ...
    private static final native JsArray<JsArrayNumber> matrix() /*-{
		return [ [ 11975, 5871, 8916, 2868 ], [ 1951, 10048, 2060, 6171 ], [ 8010, 16145, 8090, 8045 ], [ 1013, 990, 940, 6907 ] ];
    }-*/;
        ...
        return new DatumFunction<Void>() {
            ...
            public Void apply(final Element context, final Datum d, final int i) {
                svg.selectAll("." + css.chord() + " path").<Array<Chord>> cast()
                        .filter(new FilterCallback<Chord>() {
                            @Override
                            public boolean filter(final Chord d, final int index, final Array<Chord> array) {
